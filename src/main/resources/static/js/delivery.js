...
/** Set of order IDs already seen in availableOrders — used to detect genuinely NEW orders */
const seenAvailableOrderIds = new Set();
/** Tracks IDs that arrived after first load so we can animate them */
const newlyArrivedOrderIds = new Set();

// ══════════════════════════════════════════════
// 📐 HAVERSINE DISTANCE CALCULATOR
// Ported from DistanceCalculator.java
// ══════════════════════════════════════════════
...
function updateEarningsPanel() {
	...
	histEl.innerHTML = history.slice(0, 8).map((h, i) => {
      ...
      <div class="ep-delivery-item" style="animation-delay:${i * 0.05}s">
        <div class="ep-di-top">
          <div class="ep-di-id">Order #${h.order.id}</div>
          <div class="ep-di-earn">${fmtAmount(h.earning)}</div>
        </div>
        <div class="ep-di-meta">
          <span>${kmText}</span>
          <span>🕐 ${fmtDate(h.order.createdAt || h.order.updatedAt)}</span>
...
async function api(method, path, body = null) {
  ...
  console.log('API CALL =>', '/api/delivery' + path, opts.headers.Authorization);

  const res = await fetch('/api/delivery' + path, opts);

  if (res.status === 401) {
    console.log('401 from', '/api/delivery' + path);
...
// Shows alert banner + highlights new cards
// ══════════════════════════════════════════════
let newOrderAlertTimer = null;

/**
 * Called whenever we detect new orders in the available list that
 * weren't there before (i.e., a user just placed an order).
 */
function triggerNewOrderAlert(newCount, newOrders) {
	// Mark these IDs as "newly arrived" so renderAvailable can animate them
	newOrders.forEach(o => newlyArrivedOrderIds.add(o.id));

	// ── Banner alert ──
	const banner = document.getElementById('newOrderAlert');
	document.getElementById('newOrderAlertCount').textContent = newCount;
	const names = newOrders.map(o => `#${o.id}`).slice(0, 3).join(', ');
	...
	void statCard.offsetWidth; // reflow
...
/** Soft chime using Web Audio API — no external files needed */
...
function onWsConnected() {
	setWsStatus(true);
	// Subscribe to general order events (status changes, new orders, etc.)
	stompClient.subscribe('/topic/orders', (message) => {
		const event = JSON.parse(message.body);
		handleOrderEvent(event);
	});
	// Also subscribe to a dedicated "new order" topic if your backend publishes to it
	// stompClient.subscribe('/topic/orders/new', (message) => { ... });
}
 ...
 * For NEW orders (status === 'PAID'), we fetch fresh available list
 * and diff against what we had before to show the alert.
 */
function handleOrderEvent(event) {
	const isNewOrder = event.status === 'PAID' || event.type === 'NEW_ORDER' || event.type === 'ORDER_PLACED';

	if (isNewOrder) {
		// Fetch available + diff to trigger notification
		loadAvailableOrders(/* fromWsEvent */ true);
		loadMyOrders();
	} else {
		Promise.all([loadAvailableOrders(), loadMyOrders()]);
		const labels = { ASSIGNED: 'assigned', OUT_FOR_DELIVERY: 'out for delivery', DELIVERED: 'delivered' };
		const label = labels[event.status];
		if (label) showToast('ws', `📡 Order #${event.orderId} is now ${label}`);
	}
}

// ── Load Available Orders ──
/** @param {boolean} fromWsEvent - when true, diff against prev state to detect new arrivals */
async function loadAvailableOrders(fromWsEvent = false) {
	try {
		...
		if (seenAvailableOrderIds.size > 0 || fromWsEvent) {
			...
			const newOrders = freshOrders.filter(o => !seenAvailableOrderIds.has(o.id));
		...
		// Update the seen set
		freshOrders.forEach(o => seenAvailableOrderIds.add(o.id));
		// Remove IDs no longer in available (accepted by someone else)
...
function renderAvailable() {
	...
	// Flash badge green if there are new arrivals
	const hasNew = availableOrders.some(o => newlyArrivedOrderIds.has(o.id));
	...
	list.innerHTML = availableOrders.map((order, i) => {
		...
		const isNew = newlyArrivedOrderIds.has(order.id);
		const newPill = isNew ? `<span class="new-order-pill">🆕 New</span>` : '';

		return `
    <div class="order-card${isNew ? ' new-arrival' : ''}" style="animation-delay:${i * 0.06}s" data-order-id="${order.id}">
      <div class="order-top">
        <div class="order-id">Order <span>#${order.id}</span>${newPill}</div>
        ${statusBadge(order.status)}
      </div>
      <div class="order-details">
        <div class="order-row"><span class="icon">📍</span><span><strong>Deliver to:</strong> ${order.address || 'Address not set'}</span></div>
      ...
      <div class="order-meta">
        ...
        ${order.itemCount ? `<span>🛍 ${order.itemCount} item(s)</span>` : ''}
      </div>
      <div class="order-actions">
        <button class="btn btn-accept" onclick="acceptOrder(${order.id}, this)">🤝 Accept — ${earnLabel}</button>
      </div>
    </div>`;
	}).join('');

	// After cards render, settle the new-arrival animation after a short delay
	setTimeout(() => {
...
function renderMyOrders() {
    ...
    <div class="order-card" style="animation-delay:${i * 0.06}s">
      <div class="order-top"><div class="order-id">Order <span>#${order.id}</span></div>${statusBadge(order.status)}</div>
      <div class="order-details">
        <div class="order-row"><span class="icon">📍</span><span><strong>Address:</strong> ${order.address || 'Not set'}</span></div>
      ...
      ${order.address ? (isDelivered
				? `<div class="maps-link-disabled">🔒 Map Unavailable — Customer Privacy Protected</div>`
				: `<button class="maps-link-btn" onclick="openMapsWithLocation('${order.address.replace(/'/g, "\\'").replace(/"/g, '&quot;')}',this,${order.latitude ?? 'null'},${order.longitude ?? 'null'})">📍 Open in Google Maps</button>`)
				: ''}
      <div class="order-meta"><span>🕐 ${fmtDate(order.createdAt)}</span></div>
      <div class="order-actions">
        ${isAssigned ? `<button class="btn btn-start"   onclick="startDelivery(${order.id}, this)">🚀 Start Delivery</button>` : ''}
        ${isOut ? `<button class="btn btn-deliver" onclick="completeDelivery(${order.id}, this)">✅ Mark Delivered</button>` : ''}
...
async function acceptOrder(orderId, btn) {
	...
	// Remove from new arrivals set once agent acts on it
	newlyArrivedOrderIds.delete(orderId);
	try {
		const res = await api('POST', `/accept/${orderId}`);
		if (!res) return;
		if (res.ok) {
			showToast('success', '🤝 Order #' + orderId + ' accepted!');
			await Promise.all([loadAvailableOrders(), loadMyOrders()]);
		} else {
			showToast('error', '❌ ' + (await res.text() || 'Could not accept'));
...
async function startDelivery(orderId, btn) {
	btn.classList.add('btn-loading'); btn.disabled = true;
	try {
		const res = await api('POST', `/start/${orderId}`);
		if (!res) return;
		if (res.ok) {
			...
			showToast('error', '❌ ' + (await res.text() || 'Could not start delivery'));
			btn.classList.remove('btn-loading'); btn.disabled = false;
		}
	} catch {
		showToast('error', '❌ Network error.');
...
async function submitOtp() {
  ...
  try {
    ...
    const res = await fetch(`/api/delivery/complete/${pendingDeliveryOrderId}`, {
      method: 'POST',
      headers: {
        'Content-Type': 'application/json',
        ...authHeader()
    ...
    if (res.ok) {
      const order = myOrders.find(o => o.id === pendingDeliveryOrderId);
...
async function _resumeLocationSharingIfNeeded() {
	...
	try {
		...
		const activeOrder = orders.find(o => String(o.id) === String(savedOrderId) && o.status === 'OUT_FOR_DELIVERY');
		if (activeOrder) {
			showToast('info', '📡 Resuming location sharing for Order #' + savedOrderId);
			startLocationSharing(Number(savedOrderId));
		} else {
			_persistActiveOrder(null);
		}
	} catch (_) {
		const parsedId = Number(savedOrderId);
		if (parsedId) startLocationSharing(parsedId);
	}
}
...
async function stopLocationSharing() {
  ...
  if (orderIdToStop) {
    try {
      const liveToken = getToken();
      if (liveToken) {
        await fetch('/api/delivery/location/stop', {
          method: 'POST',
          headers: {
            'Content-Type': 'application/json',
            ...authHeader()
          },
          body: JSON.stringify({ orderId: orderIdToStop })
        });
      }
    } catch (_) {}
  }
...
async function sendAgentChatMessage() {
	...
	const orderSummary = myOrders.length
		? myOrders.map(o => `Order #${o.id} - ${o.status} - ${o.address || 'no address'}`).join('\n')
		: 'No active orders.';
...
document.getElementById('agentChatInput').addEventListener('keydown', e => { if (e.key === 'Enter') sendAgentChatMessage(); });
