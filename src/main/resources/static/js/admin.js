		...
		return `
            <tr>
                <td style="font-family:'Space Mono',monospace;font-size:11px;color:var(--muted)">
                    ${escapeHtml(String(u.id || '').slice(0, 8))}
                </td>
...
let stockProductsFetched = false;
...
function closeStockInventoryModal() {
	document.getElementById('stockInventoryModal').classList.remove('show');
	document.body.style.overflow = '';
}
...
function clearStockActionMessage() {
	const el = document.getElementById('stockActionMessage');
	if (!el) return;
