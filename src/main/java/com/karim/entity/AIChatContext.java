...
@Entity
@Table(name = "ai_chat_contexts", indexes = {
        @Index(name = "idx_session_id", columnList = "session_id")
})
@Data
@SQLDelete(sql = "UPDATE ai_chat_contexts SET is_deleted = true, deleted_at = now() WHERE id = ?")
...
public class AIChatContext {
    ...
    // ✅ System prompt stored as JSON
    @JdbcTypeCode(SqlTypes.JSON)
    @Column(name = "system_prompt", columnDefinition = "json")
    private Map<String, Object> systemPrompt;

    // ✅ Message history stored as JSON
    @JdbcTypeCode(SqlTypes.JSON)
    @Column(name = "message_history", columnDefinition = "json")
