...
public class AddressServiceImpl implements AddressService {
	...
	private AddressResponseDto mapToDto(Address address) {
		return AddressResponseDto.builder().id(address.getId())
				.label(address.getLabel() != null ? address.getLabel().name() : null).line1(address.getLine1())
				.line2(address.getLine2()).city(address.getCity()).state(address.getState())
				.pincode(address.getPincode()).latitude(address.getLatitude()).longitude(address.getLongitude())
				.isDefault(address.getIsDefault()).createdAt(address.getCreatedAt()).updatedAt(address.getUpdatedAt())
				.build();
	}
	...
	public void softDeleteAddress(UUID addressId, UUID userId, UUID actorId) {
	    ...
	    addressRepository.save(address); // ✅ IMPORTANT
	}
}