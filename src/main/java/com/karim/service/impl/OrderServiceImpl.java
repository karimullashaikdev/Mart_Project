...
public class OrderServiceImpl implements OrderService {
    ...
    private static final Map<OrderStatus, Set<OrderStatus>> ALLOWED_TRANSITIONS = Map.of(
            ...
            OrderStatus.PROCESSING, Set.of(OrderStatus.DISPATCHED, OrderStatus.CANCELLED),
            OrderStatus.DISPATCHED, Set.of(OrderStatus.OUT_FOR_DELIVERY),
    ...
    public Order placeOrder(UUID userId, PlaceOrderRequestDto dto, UUID actorId) {
        ...
        // If Address has getUser(), uncomment this:
        // if (!address.getUser().getId().equals(userId)) {
        //     throw new RuntimeException("Address does not belong to this user");
        // }

        // 3. Read cart summary instead of dto.items
        ...
        // 10. Clear cart after successful placement
    ...
    public Page<OrderSummaryDto> listOrdersByUser(UUID userId, OrderFilter filter, Pageable pageable) {
        ...
        Page<Order> orders = orderRepository.findOrdersByUser(
                userId,
                filter.getStatus(),
                filter.getFromDate(),
                filter.getToDate(),
        ...
        return orders.map(order -> OrderSummaryDto.builder()
                .orderId(order.getId())
                .orderNumber(order.getOrderNumber())
                .status(order.getStatus().name())
                .totalAmount(order.getTotalAmount())
    ...
    public Page<AdminOrderDto> listOrdersAdmin(OrderFilter filter, Pageable pageable) {
        ...
        Page<Order> orders = orderRepository.listOrdersAdmin(
                filter.getStatus(),
                filter.getFromDate(),
                filter.getToDate(),
                pageable
